...
class CourseLeftBlock extends Component {
    ...
    render() {
        const stars = [];
        const rating = this.props.data.leftBlock.content.rating;
        ...
        return(
            <React.Fragment>
                <div className="light-border-top light-border-bottom other-info">
                    <div className="star">
                      {stars}
                      {rating - Math.floor(rating) > 0 && <i className="fa fa-star-half-empty" key={'half'}></i>}
                      {rating <= 4 && <i className="fa fa-star-o" key={'outline'}></i>}
                    </div>
